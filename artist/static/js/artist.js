...
function buildGenreMenu() {
    ...
    genres.forEach(genre => {
        html += `
            <div
                onclick="selectGenre('${escapeHtml(genre)}')"
...
function toggleGenreMenu() {
    document
        .getElementById("genreMenu")
        .classList.toggle("show");

    document
        .getElementById("genreSelectBtn")
...
function renderTable(data, container) {
    ...
    data.forEach(artist => {
        ...
        <tr>
            ...
            <td>
                <div class="artist-cell">
                    <div
                        class="avatar"
                        style="
                        background:${getAvatarGradient(artist.nama)}
                        ">
                        ${getInitials(artist.nama)}
                    </div>

                    <span class="artist-name">
                        ${escapeHtml(artist.nama)}
                    </span>
                </div>
            </td>
            ...
            <td>
                <span class="badge">
                    ${escapeHtml(artist.genre || "Lainnya")}
                </span>
            </td>

            <td>
                <b>${artist.tampil || 0}</b> event
            </td>
    ...
    filtered.forEach(a => {
        ...
        <div class="artist-card">
            ...
            <div class="artist-card-top">
                ...
                <div
                    class="avatar-lg"
                    style="background:${getAvatarGradient(a.nama)}">
                    ${getInitials(a.nama)}
                </div>
                ...
                <div class="artist-card-info">
                    ...
                    <div class="artist-badge">
                        ${escapeHtml(a.genre || "Lainnya")}
                    </div>

                    <div class="artist-card-tampil">
                        Tampil di ${a.tampil || 0} event
                    </div>
                </div>
...
function openEdit(id) {
    ...
    fetch(
        `${EDIT_URL_BASE}${id}/data/?role=${CURRENT_ROLE}`
    )
    .then(res => res.json())
    .then(artist => {
        ...
        form.action =
            `${EDIT_URL_BASE}${id}/edit/?role=${CURRENT_ROLE}`;
...
function openDelete(id,nama){
    ...
    document.getElementById(
        ...
        `Hapus <b>${escapeHtml(nama)}</b>?`;

    document.getElementById(
        "deleteForm"
    ).action=
        `${DELETE_URL_BASE}${id}/delete/?role=${CURRENT_ROLE}`;
