...
from .forms import CustomerOrganizerRegisterForm, AdminRegisterForm
...
def register_view(request, role):
    ...
    if role not in ["organizer", "customer", "admin"]:
        messages.error(request, "Role tidak valid.")
        return redirect("accounts:pilih_role")

    form_class = AdminRegisterForm if role == "admin" else CustomerOrganizerRegisterForm
